...
const SideBar = ({ selectedUser, setSelectedUser }) => {
  ...
  return (
    <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-auto text-white ${selectedUser ?
      'max-md:hidden' : ''}`}>
      <div className='pb-5'>
        ...
        <div className='bg-[#282142] rounded-full flex items-center gap-2 py-3 px-4 mt-5'>
          ...
          <input type="text"
            className='bg-transparent border-none outline-none text-white text-xs placeholder-[#c8c8c8] flex-1' placeholder='Search User...' />
        </div>
      </div>
      ...
      <div className='flex flex-col'>
        ...
        {userDummyData.map((item,index) => (
          ...
          className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selectedUser?._id===item._id && 'bg-[#282142]/50'}`}>
            <img src={item?.profilePic || assets.avatar_icon} alt="user-pic" className='w-[35px] aspect-[1/1] rounded-full' />
